...
namespace IE.Proxy
{
    /// <summary>
    /// Use example:
    /// Restore: WinInetInterop.RestoreSystemProxy();
    /// Connect: WinInetInterop.SetConnectionProxy("162.208.49.45:3127");
    /// </summary>
...
/*
HttpWebRequest WRequest = (HttpWebRequest)HttpWebRequest.Create("https://www.google.pl/search?q=what+is+my+ip&oq=what+is+my+ip&aqs=chrome..69i57.2512j0j7&sourceid=chrome&es_sm=122&ie=UTF-8");
            WebProxy WProxy = new WebProxy("103.19.229.194", 80);
            ...
            WebBroMain.DocumentStream = stream;
*/

/* OLD: RefreshProxy("67.195.42.97:80");
 ...
 public struct INTERNET_PROXY_INFO
        ...
        private bool RefreshProxy(string sProxy)
        {
            INTERNET_PROXY_INFO IPI;
            IPI.dwAccessType = INTERNET_OPEN_TYPE_PROXY;
            IPI.proxy = Marshal.StringToHGlobalAnsi(sProxy);
            IPI.proxyBypass = Marshal.StringToHGlobalAnsi("local");

            //memory alloc
            IntPtr input = Marshal.AllocCoTaskMem(Marshal.SizeOf(IPI));

            //convert struct to IntPtr
            Marshal.StructureToPtr(IPI, input, true);
            bool bReturn = InternetSetOption(IntPtr.Zero, INTERNET_OPTION_PROXY, input, Marshal.SizeOf(IPI));
            return bReturn;
        }
